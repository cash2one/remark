...
function resetHeight(){
	...
	$('.step .push-list,.step-list').css('height', h - 70);
...
$('.step-tit .btn-count, .push-close').click(function(){
	...
	obj.animate({width: obj.is('.stepshow') ? 0 : 320}, function(){
...
// stepstart
$('.stepstart .btn-box .btn').click(function(){
...
$('.step .layoutSelect ul li').click(function(){
...
function setPad(){
	_col = Math.floor($('.step-list').width() / 100);
	$('.step-list .item').css('padding-left', '18px').filter(':nth-child(' + _col + 'n+1)').css('padding-left', '19px')
}
if($('.step-list .item').size()){
	var _col;
	setPad();
	$(window).resize(function(e) {
		if(_col != Math.floor($('.step-list').width() / 100)){
			setPad();
		};
    });
};
