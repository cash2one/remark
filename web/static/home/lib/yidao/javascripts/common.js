...
$(function(){
  ...
  if(!search || search.indexOf('reset_password') === -1) return;
  ...
  lightbox('lb1');
  scrollForm('changePwd');
});

$(function() {
  var search =  window.location.search;
  if (search && search.indexOf('next') !== -1) {
    lightbox('lb1');
    scrollForm('login');
  }
});
...
function scrollForm(target){
	switch(target){
		case 'login':
			$('#scrollForm').animate({marginLeft: 0 });
			break;
		case 'reg':
			$('#scrollForm').animate({marginLeft: -500});
			break;
		case 'changePwd':
			$('#scrollForm').animate({marginLeft: -1000});
			break;
		case 'findPwd':
			$('#scrollForm').animate({marginLeft: -1500});
			break;
		default:
			break;
	}
	// $('#scrollForm').animate({marginLeft: target == 'reg' ? -500 : -1000});
};
  ...
  alertbox.html('');
	...
	$(e).parents('.form').find('input.text').not($(e)).each(function(i,input){
		...
		var input = $(input),
			...
			findPassword = input.attr('findPassword'),
			resetPassword = input.attr('resetPassword'),
		...
		if(necessary === true && val === ''){
			...
			switch(type){
				...
				case "numLenMax8":
					reg = /^[0-9]{1,8}$/;
					errorText = "请填写8位以下正整数";
					break;
				case "numLenMax10":
					reg = /^\d{1,10}$/;
					errorText = "请填写10位以下正整数";
					break;
				case "disable": //不校验
				case "text":
					isCheck = true;
				...
				$.ajax({
			        ...
			        success: function(data){
			            ...
			            if(data.status == 400){
			            	...
			            	isCheck = true;
			            	showAlert("找回密码邮件已发送至邮箱",'success');
