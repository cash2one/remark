...
class service(base.baseService):
    def __init__(self, _model, param):
        ...
        self.paperModel = self.importModel('project_media_paper')
    ...
    def get_paper_page(self, dicData):
        listCondition = ['m.platform_id = 4']
        listJoin = []
        if dicData['category']:
        ...
        # 查询词
        if dicData['query']:
            dicField = {'1': 'name', '2': 'paper_id', '3': 'biz', '4': 'brief',
                        '5': 'contact_phone', '6': 'contact_qq', '7': 'contact_paper', '8': 'remark'}
        ...
        listJoin.append(self.dicConfig['DB_PROJECT'] + '.media_paper mw on (m.id = mw.media_id)')
        # 查询
        ...
        if tupData:
            ...
            for k, item in enumerate(tupData, 1):
                ...
                dic_paper[item['id']] = item
            # print dic_paper
            lispaper =  dic_paper.values()
            lispaper.sort(key=lambda i:i['last_update_time'], reverse=True)
        # print lispaper
        return {'paper': lispaper,
                'count': count,
    ...
    def paper_detail(self, intId):
        dicMedia = self.mediaModel.findOne({
            'condition': 'id={id}'.format(id=intId)
        })
        dicMediaPaper = self.paperModel.findOne({
        ...
        dicMediaPaper.pop('id')
        dicMedia.update(dicMediaPaper)
    ...
    def paper_sale_result(self, intId):
    ...
    def update_paper_detail(self, dicArg):
        dicpaper = {}
        if 'tag' in dicArg:
            tags = dicArg.pop('tag')
        ...
        if dicpaper:
            for k in dicpaper:
                mediapaperFields.append('{k}={v}'.format(k=k,v=dicpaper[k]))
            self.paperModel.update({
                'fields': mediapaperFields,
    ...
    def create_paper(self, dicData):
        ...
        mediaId = self.mediaModel.insert({
            ...
            'val': '%s, "%s", "%s", "%s", %s, %s, %s, %s' % (
                0, dicData['name'], dicData['brief'], dicData['avatar'], 4, 2, now, now)
        })
        self.paperModel.insert({
            'key': 'media_id, description',
            'val': '%s, "%s"' % (mediaId, dicData['description'])
        })
        if self.model.db.status != 200:
            return 500
        return 200

    def update_paper_base(self, dicData):
        now = int(self.time.time())
        self.mediaModel.update({
        ...
        self.paperModel.update({
            'fields': ['description=\'{dp}\''.format(dp=dicData['description'])],
    ...
    def update_paper(self, strId, strGender, strOriginal, strAd, strKol):
        self.mediaModel.update({
    ...
    def get_paper(self, intPage, intPageDataNum, strField, strQuery):
        dicField = {'1': 'name', '2': 'paper_id', '3': 'biz', '4': 'brief'}
        strCol = dicField[strField]
        # 搜索条件
        if strField == '1' or strField == '4':
        ...
        tupData, intRows = self.paperModel.findPaginate({
            'condition': '{search}'.format(search=searchCondition),
            'page': [intPage, intPageDataNum]
        })
        return tupData, intRows

    def update_paper(self, strId, strOriginal, strAd, strKol):
        self.paperModel.update({
            'fields': ['original={og}'.format(og=strOriginal),
                       'ad={ad}'.format(ad=strAd),
                       'kol={kol}'.format(kol=strKol)],
