...
class service(base.baseService):
    def __init__(self, _model, param):
        ...
        self.communityModel = self.importModel('project_media_community')
    ...
    def get_community_page(self, dicData):
        if dicData.get('status') == '3':
            listCondition = ['m.platform_id = 8']
        else:
            listCondition = ['m.platform_id = 8 and m.status != 3']
        listJoin = []
        if dicData['category']:
        ...
        # 查询词
        if dicData['query']:
            dicField = {'1': 'name', '2': 'community_id', '3': 'biz', '4': 'brief',
                        '5': 'contact_phone', '6': 'contact_qq', '7': 'contact_community', '8': 'remark'}
        ...
        listJoin.append(self.dicConfig['DB_PROJECT'] + '.media_community mw on (m.id = mw.media_id)')
        # 查询
        ...
        if tupData:
            ...
            for k, item in enumerate(tupData, 1):
                ...
                dic_community[item['id']] = item
            # print dic_community
            liscommunity =  dic_community.values()
            liscommunity.sort(key=lambda i:i['last_update_time'], reverse=True)
        # print liscommunity
        return {'community': liscommunity,
                'count': count,
    ...
    def get_community(self, dicData):
        listCondition = []
        listJoin = []
        # 分类
        if dicData['category']:
        ...
        # 数据格式化
        liscommunity, count = [], 0
        if tupData:
            community, count = tupData
            dic_community = {}
            for k, item in enumerate(community):
                if 'tag_id' in item:
                    item.pop('tag_id')
                item['last_update_time'] = self.formatTime(item['last_update_time'], '%Y-%m-%d')
                dic_community[item['id']] = item
    ...
    def community_detail(self, intId):
        dicMedia = self.mediaModel.findOne({
            'condition': 'id={id}'.format(id=intId)
        })
        dicMediaCommunity = self.communityModel.findOne({
        ...
        dicMediaCommunity.pop('id')
        dicMedia.update(dicMediaCommunity)
    ...
    def community_sale_result(self, intId):
    ...
    def status(self, num):
        return num, {1:'活跃', 2:'一般', 3:'僵尸'}.get(num, '-')
    ...
    def update_community_detail(self, dicArg):
        diccommunity = {}
        if 'tag' in dicArg:
            tags = dicArg.pop('tag')
        ...
        if self.model.db.status != 200:
            return 500
        mediacommunityFields = []
        if diccommunity:
            for k in diccommunity:
                mediacommunityFields.append('{k}={v}'.format(k=k,v=diccommunity[k]))
            self.communityModel.update({
                'fields': mediacommunityFields,
    ...
    def check_biz(self, biz):
        res = self.communityModel.findOne({
            'condition': 'biz="%s"' % biz
        })
        if res:
            return 601
    ...
    def create_community(self, dicData):
        ...
        mediaId = self.mediaModel.insert({
            ...
            'val': '%s, "%s", "%s", "%s", %s, %s, %s, %s' % (
                0, dicData['name'], dicData['brief'], dicData['avatar'], 8, 2, now, now)
        })
        self.communityModel.insert({
            'key': 'media_id, population',
            'val': '%s, "%s"' % (mediaId, dicData['population'])
        })
        if self.model.db.status != 200:
            return 500
        return 200

    def update_community_base(self, dicData):
        now = int(self.time.time())
        self.mediaModel.update({
        ...
        self.communityModel.update({
            'fields': ['population={dp}'.format(dp=dicData['population'])],
    ...
    def update_community(self, strId, strGender, strOriginal, strAd, strKol):
        self.mediaModel.update({
    ...
    def get_community(self, intPage, intPageDataNum, strField, strQuery):
        dicField = {'1': 'name', '2': 'community_id', '3': 'biz', '4': 'brief'}
        strCol = dicField[strField]
        # 搜索条件
        if strField == '1' or strField == '4':
        ...
        tupData, intRows = self.communityModel.findPaginate({
            'condition': '{search}'.format(search=searchCondition),
            'page': [intPage, intPageDataNum]
        })
        return tupData, intRows

    def update_community(self, strId, strOriginal, strAd, strKol):
        self.communityModel.update({
            'fields': ['original={og}'.format(og=strOriginal),
                       'ad={ad}'.format(ad=strAd),
                       'kol={kol}'.format(kol=strKol)],
