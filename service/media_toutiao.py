...
class service(base.baseService):
    def __init__(self, _model, param):
        ...
        self.toutiaoModel = self.importModel('project_media_toutiao')
    ...
    def get_toutiao_page(self, dicData):
        listCondition = ['m.platform_id = 9']
        listJoin = []
        if dicData['category']:
        ...
        if dicData['avg_read_num'] and dicData['avg_read_num'] != ',':
            acn = self.build_interval_condition('mw.avg_read_num', dicData['avg_read_num'])
            if acn: listCondition.append(acn)
        if dicData['avg_comment_num'] and dicData['avg_comment_num'] != ',':
            tvn = self.build_interval_condition('mw.avg_comment_num', dicData['avg_comment_num'])
            if tvn: listCondition.append(tvn)
        if dicData['post_price'] and dicData['post_price'] != ',':
            fp = self.build_interval_condition('mw.post_price', dicData['post_price'])
        ...
        # 查询词
        if dicData['query']:
            dicField = {'1': 'name', '2': 'toutiao_id', '3': 'biz', '4': 'brief',
                        '5': 'contact_phone', '6': 'contact_qq', '7': 'contact_toutiao', '8': 'remark'}
        ...
        listJoin.append(self.dicConfig['DB_PROJECT'] + '.media_toutiao mw on (m.id = mw.media_id)')
        # 查询
        ...
        if tupData:
            ...
            for k, item in enumerate(tupData, 1):
                ...
                dic_toutiao[item['id']] = item
            # print dic_toutiao
            listoutiao =  dic_toutiao.values()
            listoutiao.sort(key=lambda i:i['last_update_time'], reverse=True)
        return {'toutiao': listoutiao,
                'count': count,
    ...
    def get_toutiao(self, dicData):
        ...
        listoutiao, count = [], 0
        if tupData:
            toutiao, count = tupData
            dic_toutiao = {}
            for k, item in enumerate(toutiao):
                if 'tag_id' in item:
                    item.pop('tag_id')
                item['last_update_time'] = self.formatTime(item['last_update_time'], '%Y-%m-%d')
                dic_toutiao[item['id']] = item
    ...
    def toutiao_detail(self, intId):
        dicMedia = self.mediaModel.findOne({
            'condition': 'id={id}'.format(id=intId)
        })
        dicMediaToutiao = self.toutiaoModel.findOne({
        ...
        dicMediaToutiao.pop('id')
        dicMedia.update(dicMediaToutiao)
    ...
    def toutiao_sale_result(self, intId):
    ...
    def update_toutiao_detail(self, dicArg):
        dictoutiao = {}
        if 'tag' in dicArg:
            tags = dicArg.pop('tag')
        ...
        if 'avg_read_num' in dicArg:
            dictoutiao['avg_read_num'] = dicArg.pop('avg_read_num')
        if 'avg_comment_num' in dicArg:
            dictoutiao['avg_comment_num'] = dicArg.pop('avg_comment_num')
        if 'post_price' in dicArg:
            dictoutiao['post_price'] = dicArg.pop('post_price')
        ...
        if dictoutiao:
            for k in dictoutiao:
                mediatoutiaoFields.append('{k}={v}'.format(k=k,v=dictoutiao[k]))
            self.toutiaoModel.update({
                'fields': mediatoutiaoFields,
    ...
    def check_url(self, url):
        if not url.startswith('http://toutiao.com/m'):
            return 401
        return 200
    ...
    def check_url_update(self, mid, url):
        if not url.endswith('/'):
            url = url + '/'
        res = self.toutiaoModel.findOne({
        ...
        url_ex = res.get('url', '')
        if url_ex and url_ex != url:
            return 601
        return 200

    def create_toutiao(self, dicData):
        ...
        mediaId = self.mediaModel.insert({
            ...
            'val': '%s, "%s", "%s", "%s", %s, %s, %s, %s' % (
                0, dicData['name'], dicData['brief'], dicData['avatar'], 9, 2, now, now)
        })
        self.toutiaoModel.insert({
            'key': 'media_id, url',
            'val': '%s, "%s"' % (mediaId, dicData['url'])
        })
        if self.model.db.status != 200:
            return 500
        return 200

    def update_toutiao_base(self, dicData):
        now = int(self.time.time())
        self.mediaModel.update({
    ...
    def update_toutiao(self, strId, strGender, strOriginal, strAd, strKol):
        self.mediaModel.update({
    ...
    def get_toutiao(self, intPage, intPageDataNum, strField, strQuery):
        dicField = {'1': 'name', '2': 'toutiao_id', '3': 'biz', '4': 'brief'}
        strCol = dicField[strField]
        # 搜索条件
        if strField == '1' or strField == '4':
        ...
        tupData, intRows = self.toutiaoModel.findPaginate({
            'condition': '{search}'.format(search=searchCondition),
            'page': [intPage, intPageDataNum]
        })
        return tupData, intRows

    def update_toutiao(self, strId, strOriginal, strAd, strKol):
        self.toutiaoModel.update({
            'fields': ['original={og}'.format(og=strOriginal),
                       'ad={ad}'.format(ad=strAd),
                       'kol={kol}'.format(kol=strKol)],
