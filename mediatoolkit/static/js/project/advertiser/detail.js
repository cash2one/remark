$(function(){
    ...
    $(".table-info").each(function(index, el) {
        ...
        switch (index) {
            // 基本信息
            case 0:
            ajaxUrl('basic',$this,companyInfo,$id);
            break;

            // 联系方式
            case 1:
            $.ajax({
                url: '/project/advertiser?a=get_contact&id=' + $id,
                method: 'get',
                dataType: 'json',
                // data: $id,
                beforeSend: function(){
            ...
            // 公司信息
            case 2:
            ajaxUrl('text',$this,advertiser,$id);
            break;

            // 负责人
            case 3:
            ajaxUrl('follower',$this,follower,$id);
            break;

            // 增加计划
            case 4:
            ajaxUrl('plan',$this,'',$id);
            break;
        }
    });
    // 修改广告主基本信息
    ...
    $("#advertiser_follow_btn").click(function(event) { 
        $.getJSON("/project/advertiser?a=get_user&id=" + $id,function(json){
            if(json.status == 200){
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            if (result.status ==200){
                window.location.href="/project/advertiser?a=detail&id=" + $id + "&type=look";
...
function on_advertiser_basic_commit($id){
    ...
    $.ajax({
        url: "/project/advertiser?a=update_basic",
        method: "post",
        dataType:'json',
        data: dicArgs,
        success: function (result) {
...
// 请求所有表格数据
function ajaxUrl(getUrl,_this,dataName,$id){
    $.ajax({
        url: '/project/advertiser?a=get_' + getUrl + '&id='+ $id,
        method: 'post',
        dataType: 'json',
        data: $id,
        beforeSend: function(){
...
function advertiser_basic_revise($id) {
    $.getJSON('/project/advertiser?a=get_basic&id='+ $id, function(json) {
        ...
        $("#advertiser_basic_form .edui-body-container").html(brief);
...
function add_contact($id){
    ...
    $.ajax({
        url: "/project/advertiser?a=add_contact",
        method: "get",
        dataType:'json',
        data: dicArgs,
        success: function (result) {
...
function contact_revise($id){
    $(".contact_revise").click(function() {
        ...
        $.getJSON('/project/advertiser?a=get_contact&id=' + $id,function(json) {
            for(var i in json.data.detail_info_value){
...
function update_contact_advertiser(name_id,$id){
    ...
    $.ajax({
        url: "/project/advertiser?a=update_contact",
        method: "post",
        dataType:'json',
        data: dicArgs,
...
function delete_contact($id){
    $(".btn-danger").click(function(event) {
        ...
        if(confirm("您确定要删除此联系人吗")){
            ...
            $.ajax({
                url: '/project/advertiser?a=delete_contact',
                method: 'get',
                dataType: 'json',
                data: $id,
                success: function(json){
...
function updata_follower($id){
    ...
    $("#advertiser_follower_revise_form input:checked").each(function(index,el) {
        var $follower_id = $(el).val();
        follower_id[index] = $follower_id;
    });
    var dicArgs = {
            "id":$id,
    ...
    $.ajax({
        url: '/project/advertiser?a=update_follower',
        dataType: 'json',
        data: dicArgs,
        success: function(result){
            if (result.status == 200){
