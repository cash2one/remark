...
$(function(){
	...
	//自媒体筛选工具
	var i=0;
	$(".mediaCart .dick").click(function(){
		i+=1;
		if(!(i%2)==0){
			$(".mediaCart .dick").html("+");
			$(".mediaCart .mediaList").hide();
			$(".mediaCart").addClass("mediaCartMin")
		}
		else {
			$(".mediaCart .dick").html("-");
			$(".mediaCart .mediaList").show();
			$(".mediaCart").removeClass("mediaCartMin")
		}
	});
})
...
function time_compare(a, b){
  ...
  *  str = 编辑框id 此只针对模态框大小
 */
function uediter(str){
    window.UMEDITOR_HOME_URL = "../../../static/ueditor/";
    window.um = UM.getEditor(str, {
