...
class base(controller):
    ...
    def page(self, intPage, intPageDataNum, intDataCount, strPageUrl):
        ...
        if intPageCount > 1:
            ...
            lisPageHtml.append('</ul><div class="searchBox">'
                               ...
                               '<button id="pageGo" class="btn btn-primary" style="margin-left:10px;">Go</button></div></nav>' % intPageCount)
    ...
    def get_current_user(self):
        ...
        if userId:
            access, self.isGod = self.importService('admin_access').getUserModuleName(userId)
        else:
            access = ''
    ...
    def access(self):
        ...
        strModuelName = self.__module__.split('.')[1]
        ...
        # print " 11 argument = ", status, self.request.arguments, strActionName, self.request.path, strModuelName, strClassName

        # 不在权限表中strFileName=('0', '0')，则不需要权限控制
        if self.isGod or '0' in strFileName :
            return self.user_access_module(userId, strFileName[0], url)

        status, action = self.importService("admin_permission_manager").checkAction(strFileName[1], strActionName)
        if not status:
            if strActionName == 'index' or strActionName == 'error'  or str(strFileName[0]) in accessSet :
                return self.user_access_module(userId, strFileName[0], url)
        else:
            if str(strFileName[0]) in accessSet and str(action['id']) in accessSet:
                return self.user_access_module(userId, strFileName[0], url)


        controller.operatStatus=1
        data = str(self.request)
        if "X-Requested-With" in data:
            self.out(403)
            return False

        try:
            view = [ i.split('.')[0] for i in (os.listdir(sys.path[0]+'/view/'+self.request.path[1:]))]
        except :
            view = []

        if self.request.arguments:
            if len(self.request.path.split('/'))==3:
                if 'index' in view:
                    self.display('index', self.request.path.split('/')[1],self.request.path.split('/')[2])
                    return False
                self.display(strClassName, self.request.path.split('/')[1],self.request.path.split('/')[2])
            else:
                self.display('index', '', self.request.path.split('/')[1])
        else:
            self.display('index', '', 'index')
        return False
